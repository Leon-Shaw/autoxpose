...
export function CleanupSection(): JSX.Element {
  ...
  if (isLoading) {
    return (
      <div className="rounded-md border border-[#30363d] bg-[#161b22] p-4">
        <h4 className="mb-3 text-sm font-semibold text-[#c9d1d9]">{t('orphans.title')}</h4>
        <p className="text-xs text-[#8b949e]">{t('orphans.loading')}</p>
      </div>
    );
  }
  ...
  if (orphans.length === 0) {
    return (
      <div className="rounded-md border border-[#30363d] bg-[#161b22] p-4">
        <h4 className="mb-3 text-sm font-semibold text-[#c9d1d9]">{t('orphans.title')}</h4>
        <p className="text-xs text-[#8b949e]">{t('orphans.services_created_no_container')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-md border border-[#30363d] bg-[#161b22] p-4">
      <h4 className="mb-3 text-sm font-semibold text-[#c9d1d9]">
        {t('orphans.orphaned_resources_count', { count: orphans.length })}
      </h4>
      <p className="mb-3 text-xs text-[#8b949e]">{t('orphans.services_no_longer_exist')}</p>
