...
export function ServicesDisplay({ services }: ServicesDisplayProps): JSX.Element | null {
  ...
  if (serviceCount === 0) {
    return (
      <div className="rounded-lg border border-[#30363d] bg-[#0d1117] px-4 py-2 text-center text-sm text-[#8b949e]">
        {t('topology.no_services_exposed')}
      </div>
    );
  }
  ...
  if (!servicesExpanded) {
    return (
      ...
      >
        <div className="font-mono text-[#58a6ff]">{t('topology.services_count', { count: serviceCount })}</div>
        <div className="text-xs text-[#8b949e]">{t('topology.click_to_expand')}</div>
      </button>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {hasMore && (
        <div className="text-center text-xs text-[#8b949e]">{t('topology.more_services', { count: serviceCount - 6 })}</div>
      )}
    </div>
  );
}
